...
/* ─────────────────────────────────────────────────────────────
   CUSTOM DROPDOWN (with status pills + keyboard nav)
───────────────────────────────────────────────────────────── */
function Dropdown({
  value,
  onChange,
...
/* ─────────────────────────────────────────────────────────────
   SAFE DEVICE LABEL EXTRACTOR — guarantees a non-empty label
───────────────────────────────────────────────────────────── */
function extractDeviceLabel(device, index) {
  ...
  /* Last resort: pick any short non-empty string from the object */
  if (typeof device === 'object') {
    const fallback = Object.values(device).find(
      (v) =>
        typeof v === 'string' &&
        v.trim() !== '' &&
        v.length <= 80 &&
        !['up', 'down', 'active', 'inactive'].includes(v.toLowerCase())
    )
...
function AddForm({ branches, onAdd }) {
  ...
  /* 🟢 Guaranteed non-empty labels + status pills */
  const deviceOptions = (fetchedDevices || []).map((device, index) => {
    const label = extractDeviceLabel(device, index)

    const status =
      typeof device === 'object' && device !== null
        ? device.agentStatus ?? device.status ?? device.agent_status ?? null
        : null
  ...
  const set = (key) => (value) => {
    setForm((prev) => ({
      ...prev,
      [key]: value,
      ...(key === 'branch' ? { device: '', function: '' } : {}),
    }))
  }

  const isValid =
    form.branch && form.device && form.function && form.drivePath

  /* Branch change → fetch devices */
  const handleBranchChange = async (branch) => {
    ...
    try {
      ...
      setFetchedDevices(Array.isArray(data) ? data : [])
    } catch (error) {
      console.error('Error loading devices for branch:', error)
      setFetchedDevices([])
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Add Drive Policy Response:', response)
      ...
      console.error('Error adding drive policy:', error)

      showAlert?.('Unable to add drive policy.')
    }
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        ...
        <div>
          ...
          <Dropdown
            ...
            disabled={!form.branch || loadingDevices}
            searchable
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) => set('drivePath')(e.target.value)}
              ...
              className={`
                ...
                ${submitted && !form.drivePath ? 'ring-2 ring-rose-500/20' : ''}
              `}
              style={{
                background: isDark ? '#111827' : 'rgba(255,255,255,0.80)',
                backdropFilter: 'blur(24px)',
                WebkitBackdropFilter: 'blur(24px)',
              }}
            />
...
function PolicyTable({ policies, onDelete }) {
  ...
  const filtered = safePolicies.filter((p) => {
    ...
    const branch = String(getValue(p, 'branch', 'branchName')).toLowerCase()

    const device = String(
      getValue(p, 'device', 'deviceName', 'computerName', 'hostName', 'hostname')
    ).toLowerCase()

    const ipAddress = String(
      getValue(p, 'ipAddress', 'ipaddress', 'ip')
    ).toLowerCase()
    ...
    <GlassCard className="overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div
            className={`
              relative flex items-center rounded-xl border text-[12px]
          ...
          >
            ...
            <input
              ...
              placeholder="Search..."
              className={`
                bg-transparent pl-7 pr-3 py-1.5 outline-none w-44 text-[12px]
                ${
                  isDark
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className={isDark ? 'bg-white/[0.015]' : 'bg-slate-50/70'}>
                {['#', 'Branch', 'Device', 'Function', 'Drive Path'].map(
                  (h, i) => (
                    <th
                      key={h}
                      className={`
            ...
            <tbody>
              {filtered.map((p, i) => {
                const rowId =
                  ...
                  p.srNo ??
                  p.sr_no ??
                  `${getValue(p, 'branch', 'branchName')}-${getValue(
                    p,
                    'device',
                    'deviceName',
                    'computerName',
                    'hostName',
                    'hostname'
                  )}-${getValue(p, 'function', 'functions', 'FUNCTIONS')}-${getValue(
                    p,
                    'drivePath',
                    'drivepath',
                    'path'
                ...
                const branch = getValue(p, 'branch', 'branchName')
                ...
                const func = getValue(p, 'function', 'functions', 'FUNCTIONS')
                const drivePath = getValue(p, 'drivePath', 'drivepath', 'path')
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          className="
                            ...
                            bg-[#7094ff]/10 border border-[#7094ff]/15
                          "
                        >
                          <Folder size={11} className="text-[#7094ff]" />
                        </div>

                        <span className="truncate max-w-[280px]">
                          {drivePath ||
                            getValue(p, 'mode', 'modeOfAccess', 'mode_access') ||
                            'N/A'}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        title="Delete Policy"
                        className="
                          w-7 h-7 rounded-lg flex items-center justify-center ml-auto
...
export default function DriveControl() {
  ...
  const createPolicyId = (policy, index) => {
    if (policy?.id !== undefined && policy?.id !== null) return policy.id
    if (policy?.policyId !== undefined && policy?.policyId !== null)
      return policy.policyId
    if (policy?.srNo !== undefined && policy?.srNo !== null) return policy.srNo
    if (policy?.sr_no !== undefined && policy?.sr_no !== null) return policy.sr_no

    return [
      policy?.branch ?? policy?.branchName ?? 'branch',
      policy?.ipAddress ?? policy?.ipaddress ?? 'ip',
      policy?.device ?? policy?.deviceName ?? policy?.computerName ?? 'device',
      policy?.function ?? policy?.functions ?? 'function',
      policy?.drivePath ?? policy?.drivepath ?? 'path',
      index,
    ].join('-')
  }

  const handleAdd = ({ branch, branchName, device, function: func, drivePath }) => {
    const newPolicy = {
  ...
  const loadPageData = async () => {
    try {
      ...
      const [ALLBranch, DrivePolicies] = await Promise.all([
        dashboardService.getBranch(),
        dashboardService.getDrivePolicies(),
      ])

      const branchData = Array.isArray(ALLBranch?.data) ? ALLBranch.data : []
      setBranches(branchData)

      const policyData = Array.isArray(DrivePolicies?.data)
        ? DrivePolicies.data
        : []

      const normalizedPolicies = policyData.map((policy, index) => ({
        ...policy,
        id: createPolicyId(policy, index),
      }))

      setPolicies(normalizedPolicies)
    } catch (err) {
      console.error('Error loading drive protection data:', err)
      setError('Unable to load drive policy data.')
      setPolicies([])
      setBranches([])
    } finally {
      setLoading(false)
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-start justify-between mb-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p
              ...
              `}
            >
              Manage drive access policies across endpoints
            </p>
          </div>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-2">
          ...
          <GlassButton
            ...
            className="px-3 py-1.5 text-[11px] font-semibold cursor-default"
          >
            {policies.filter((p) => p.drivePath || p.drivepath || p.path).length}{' '}
            Active
          </GlassButton>
        </div>
      </div>
      ...
      {error && (
        <div
          className="
            mb-5 rounded-xl border border-rose-500/20
            bg-rose-500/10 px-4 py-3 text-[12px] text-rose-400
          "
        >
          {error}
        </div>
      ...
      {loading ? (
        <GlassCard className="p-10">
          <div className="flex flex-col items-center justify-center gap-3">
            <div
              className="
                w-8 h-8 border-2 border-[#7094ff]/20 border-t-[#7094ff]
