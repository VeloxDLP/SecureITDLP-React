...
import {
  ...
  ShieldCheck, Trash2, Search, ChevronDown, X, Folder,
} from 'lucide-react'
...
function Dropdown({
  ...
  const glassSurface = isDark
    ? { background: '#171827', backdropFilter: 'none', WebkitBackdropFilter: 'none' }
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div
          ...
          `}
          style={{
            background: isDark ? '#171827' : 'rgba(255,255,255,0.98)',
...
function Badge({ mode }) {
  ...
  if (mode === 'Allow' || mode === 'Read' || mode === 'Read/Write' || mode === 'Full Control')
    return (
...
function GlassCard({ children, className = '' }) {
  ...
  return (
    <div
      ...
      style={{
        background: isDark ? '#171827' : 'rgba(255,255,255,0.95)',
...
function AddForm({ branches, onAdd }) {
  ...
  const { isDark } = useTheme()
  const [form, setForm] = useState({ branch: '', device: '', function: '', drivePath: '' })
  const [submitted, setSubmitted] = useState(false)
  const [success, setSuccess] = useState(false)
  const [fetchedDevices, setFetchedDevices] = useState([]);
  const set = k => v =>
    setForm(f => ({ ...f, [k]: v, ...(k === 'branch' ? { device: '', function: '' } : {}) }))
  ...
  const isValid = form.branch && form.device && form.function && form.drivePath
  ...
  return (
    <GlassCard className="p-6 mb-5">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        ...
        <div>
          <label className={labelCls}>
            Functions <span className="text-rose-500 normal-case tracking-normal">*</span>
          </label>
          <Dropdown
            value={form.function}
          ...
          />
          {submitted && !form.function && <p className="text-[10px] text-rose-500 mt-1">Required</p>}
        </div>

        {/* Drive Path */}
        <div>
          <label className={labelCls}>
            Drive Path <span className="text-rose-500 normal-case tracking-normal">*</span>
          </label>
          <div className="relative">
            <input
              ...
              className={`
                w-full px-3 py-2.5 pl-10 rounded-xl text-[13px]
                border transition-all duration-200 outline-none
                ${submitted && !form.drivePath ? 'border-rose-500/60' : isDark ? 'border-white/[0.10]' : 'border-slate-300/70'}
                ${isDark ? 'bg-[#171827] text-slate-200 placeholder-slate-500' : 'bg-white/80 text-slate-800 placeholder-slate-400'}
                ${submitted && !form.drivePath ? 'ring-2 ring-rose-500/20' : ''}
              `}
              style={{
                background: isDark ? '#171827' : 'rgba(255,255,255,0.80)',
          ...
          {submitted && !form.drivePath && <p className="text-[10px] text-rose-500 mt-1">Required</p>}
        </div>
      ...
      <div className="flex items-center justify-end gap-3">
        <GlassButton
          onClick={() => { setForm({ branch: '', device: '', function: '', drivePath: '' }); setSubmitted(false) }}
          variant="default"
          className="px-4 py-2"
        >
  ...
  const filtered = policies.filter(p => {
    ...
    return (
      (!q || 
        ...
        (p.drivePath && p.drivePath.toLowerCase().includes(q)) ||
        (p.function && p.function.toLowerCase().includes(q))
      ) &&
      (!filterMode || p.function === filterMode)
    )
  })
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <th className={thCls}>Drive Path</th>
              ...
              {filtered.map((p, i) => (
                ...
                >
                  ...
                  <td className={`${tdCls} font-mono text-[11px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    {p.ipAddress || p.device || 'N/A'}
                  </td>
                  <td className={tdCls}>
                    <span className={`px-2 py-1 rounded-md text-[11px] font-medium
                      ${p.function === 'CREATE' ? 'bg-emerald-500/10 text-emerald-500' :
                        p.function === 'MODIFY' ? 'bg-blue-500/10 text-blue-500' :
                        p.function === 'DELETE' ? 'bg-rose-500/10 text-rose-500' :
                        p.function === 'RENAME' ? 'bg-amber-500/10 text-amber-500' :
                        p.function === 'CUT' ? 'bg-purple-500/10 text-purple-500' :
                  ...
                  <td className={`${tdCls} font-mono text-[11px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    <span className="flex items-center gap-1.5">
                      <Folder size={12} className="text-[#7094ff]" />
                      {p.drivePath || p.mode || 'N/A'}
                    </span>
                  </td>
...
function TabBar({ active, onChange }) {
  ...
  return (
    <div
      ...
      style={{
        background: isDark ? '#171827' : 'rgba(255,255,255,0.60)',
        backdropFilter: 'blur(20px)',
        WebkitBackdropFilter: 'blur(20px)',
...
export default function DriveControl() {
  ...
  const handleAdd = ({ branchName, device, function: func, drivePath }) => {
    setPolicies(prev => [
      ...prev,
      {
        id: Date.now(),
      ...
      <div className="flex items-start justify-between mb-7">
        ...
        <div className="hidden sm:flex items-center gap-2">
          ...
          <GlassButton variant="chip_prevent" className="px-3 py-1.5 text-[11px] font-semibold cursor-default">
            {policies.filter(p => p.drivePath).length} Active
          </GlassButton>
        </div>
      </div>
