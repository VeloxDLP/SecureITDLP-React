...
const incidentColumns = [
  ...
  { accessor: "hostName", header: "HOST NAME" },
  { accessor: "attachments", header: "ATTACHMENTS" },
    ...
    return (
      ...
      >
        ...
        <p className="text-xs text-slate-500 dark:text-white/60">
          Incidents: <span className="font-semibold">{payload[0].value}</span>
        </p>
      </div>
    );
  }
...
export default function MailIncidentChart({
  ...
  const totalIncidents = useMemo(() => {
    return chartData.reduce((sum, item) => sum + Number(item?.v || 0), 0);
  }, [chartData]);
  ...
  const handleDateClick = async (clickedDate) => {
    ...
    try {
      const response = await dashboardService.getMailIncidentData(clickedDate);
