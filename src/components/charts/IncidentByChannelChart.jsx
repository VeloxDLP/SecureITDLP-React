...
const getChannelValue = (data, channel) => {
  ...
  const normalizedChannel = channel.replace(/[_\-\s]/g, "").toLowerCase();
  const matchedKey = keys.find((key) => {
    const normalizedKey = String(key).replace(/[_\-\s]/g, "").toLowerCase();
    return normalizedKey === normalizedChannel || normalizedKey.includes(normalizedChannel);
  });
...
const IncidentByChannel = ({ data = {} }) => {
  ...
  /* =======================================================
     MODAL TRANSITION STATES (matching file‑type modal)
  ======================================================= */
  ...
  const channelData = useMemo(() => {
    return [
      { name: "Email", value: getChannelValue(data, "Email"), color: CHANNEL_COLORS.Email },
      { name: "Network", value: getChannelValue(data, "Network"), color: CHANNEL_COLORS.Network },
      { name: "Printer", value: getChannelValue(data, "Printer"), color: CHANNEL_COLORS.Printer },
      { name: "Clipboard", value: getChannelValue(data, "Clipboard"), color: CHANNEL_COLORS.Clipboard },
      { name: "Drive", value: getChannelValue(data, "Drive"), color: CHANNEL_COLORS.Drive },
      { name: "Peripherals", value: getChannelValue(data, "Peripheral"), color: CHANNEL_COLORS.Peripherals },
    ];
  }, [data]);

  const totalIncidents = channelData.reduce((sum, item) => sum + Number(item.value || 0), 0);

  /* =======================================================
     OPEN MODAL (with mounting + animation)
  ======================================================= */
  const openChannelModal = async (channel) => {
    ...
    setModalData([]);
    setLoading(true);
    setIsMounted(true); // mount DOM

    try {
      const response = await dashboardService.getIncidentByChannelModal(apiChannel);
      setModalData(Array.isArray(response?.data) ? response.data : []);
    } catch (err) {
  ...
  const filteredData = useMemo(() => {
    ...
    return modalData.filter((row) => {
      ...
      if (Array.isArray(row)) {
        return row.some((value) => String(value ?? "").toLowerCase().includes(searchValue));
      }
  ...
  const firstRecord = filteredData.length === 0 ? 0 : startIndex + 1;
  const lastRecord = Math.min(startIndex + rowsPerPage, filteredData.length);

  /* =======================================================
     TABLE HEADERS (per channel)
  ======================================================= */
  const getTableHeaders = () => {
    if (selectedChannel === "Drive") {
      return ["BRANCH", "IP ADDRESS", "PC NAME", "DATE"];
    }
    if (selectedChannel === "Printer") {
      return ["BRANCH", "DATE", "HOST NAME", "JOB STATUS"];
    }
    return ["BRANCH", "USERNAME", "EVENT TYPE", "DEVICE / FILE DETAILS", "TIMESTAMP"];
  };
  ...
  return (
    <>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                onClick={(entry) => {
                  if (entry?.name) openChannelModal(entry.name);
                }}
              >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-2">
          {channelData.map((item) => (
            <button
              ...
              className="flex w-full items-center justify-between rounded-md px-2 py-1 text-left transition-all duration-200 ease-out hover:bg-slate-100 hover:translate-x-[2px] dark:hover:bg-white/[0.04]"
            >
              <div className="flex items-center gap-2">
                <span className="h-[8px] w-[8px] rounded-full" style={{ backgroundColor: item.color }} />
                <span className="text-[11px] text-slate-600 dark:text-white/60">{item.name}</span>
              </div>
              <span className="text-[11px] font-semibold text-slate-700 dark:text-white/80">{item.value}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* =====================================================
          MODAL – with same styling & transitions as file‑type modal
      ===================================================== */}
      {isMounted && (
        <div
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 flex items-center gap-2">
                  <span
                    ...
                    style={{ backgroundColor: CHANNEL_COLORS[selectedChannel] || "#3B5BFF" }}
                  />
                  <span className="text-[11px] text-slate-500 dark:text-white/40">{selectedChannel}</span>
                </div>
              </div>
                ...
                <div className="min-h-0 flex-1 overflow-auto">
                  {loading ? (
                    <div className="flex h-full min-h-[300px] flex-col items-center justify-center gap-3">
                      <Loader2 size={25} className="animate-spin text-[#7094ff]" />
                    ...
                    <div className="flex h-full min-h-[300px] flex-col items-center justify-center">
                      ...
                      <button
                        type="button"
                        onClick={() => openChannelModal(selectedChannel)}
                      ...
                      <tbody>
                        {currentRows.length > 0 ? (
                          currentRows.map((row, index) => {
                            // ---------- PRINTER ----------
                            if (selectedChannel === "Printer") {
                              const branch = row?.branch ?? row?.branchName ?? "NA";
                              const date = row?.date ?? row?.cdate ?? "NA";
                              const hostName = row?.hostName ?? row?.hostname ?? "NA";
                              const jobStatus = row?.jobStatus ?? row?.jobstatus ?? "NA";
                              const isBlocked =
                                String(jobStatus).toLowerCase().includes("prevent") ||
                                String(jobStatus).toLowerCase().includes("fail") ||
                                String(jobStatus).toLowerCase().includes("block");
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="max-w-[300px] truncate border-b border-slate-100 px-4 py-3 font-medium text-sky-600 dark:border-white/[0.05] dark:text-sky-400">
                                    <div className="truncate" title={hostName}>
                                      {hostName}
                                    </div>
                                  </td>
                            ...
                            // ---------- DRIVE ----------
                            if (selectedChannel === "Drive") {
                              const branch = row?.branchName || "NA";
                              const ipAddress = row?.ipAddress || "NA";
                              const pcName = row?.pcName || "NA";
                              const cdate = row?.cdate || null;
                            ...
                            // ---------- OTHER CHANNELS ----------
                            const branch = Array.isArray(row) ? row[0] : row?.branchName || "NA";
                            const username = Array.isArray(row) ? row[3] : row?.username || "NA";
                            const eventType = Array.isArray(row) ? row[1] : row?.eventType || "NA";
                            const device = Array.isArray(row)
                              ? row[2]
                              : row?.deviceName || row?.ipAddress || "NA";
                            const timestamp = Array.isArray(row)
                              ? row[4]
                              : row?.cdate || row?.timestamp || null;
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="max-w-[430px] truncate border-b border-slate-100 px-4 py-3 text-slate-400 dark:border-white/[0.05] dark:text-white/45">
                                  <div className="truncate" title={String(device)}>
                                    {device}
                                  </div>
                                </td>
                          ...
                          <tr>
                            ...
                            >
                              {search ? "No matching incidents found" : `No ${selectedChannel} incident records found`}
                            </td>
                          </tr>
                        )}
                      </tbody>
                ...
                {!loading && !error && (
                  <div className="flex shrink-0 flex-col gap-1 border-t border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/[0.08] dark:bg-white/[0.02] sm:flex-row sm:items-center sm:justify-between">
                    <span className="text-[10px] text-slate-500 dark:text-white/40">
                      Showing {firstRecord}-{lastRecord} of {filteredData.length}
                    </span>
